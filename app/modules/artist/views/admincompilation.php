...
<div class="g-content">
    <div class="b-edit-release">
        ...
        <div class="upload">
            <div class="upload-button"><?=$this->registry->trans['add'];?> <?=strtolower($this->registry->trans['compilation']);?> <b><?=$this->view->releasetype['name'];?></b> (*.mp3)</div>
...
<div id="adding-popup">
    <span class="label-author"><?=$this->registry->trans['author'];?> </span><span class="label-name" style="width:450px;"><?=$this->registry->trans['title'];?> </span><span class="label-style"><?=$this->registry->trans['style'];?>  (Eng.)</span>

    <div>
        <input class="author" type="text" value="<?=$this->view->releasetype['author'];?>" /><input class="name" type="text" style="width:420px;" /><input class="style" type="text" />
    </div>

    <div class="options">
        <label><input class="download" type="checkbox" /> <?=$this->registry->trans['downloadable'];?> </label><label><input class="playlist" type="checkbox" /> <?=$this->registry->trans['addtoplay'];?> </label>
    </div>
...
<script type="text/javascript">
    $().ready(function() {
        var release = $.parseJSON('<?=$this->view->compilationlist;?>'),
        ...
        function editTrack(mp3, track) {
            addBlock.find('.author').val('<?=tools::tojs($this->view->author);?>');
         ...
         function packJsonAndSend() {
            ...
            itemsBlock.find('li:visible').each(function(i) {
                release.compilation[i] = {};

                release.compilation[i].id = $(this).data('id');

                if ($(this).data('fileid')) {
                    release.compilation[i].fileid = $(this).data('fileid');
                };

                if ($(this).data('mp3')) {
                    release.compilation[i].url = $(this).data('mp3');
                };

                if ($(this).data('stream')) {
                    release.compilation[i].stream = $(this).data('stream');
                };

                if ($(this).data('socialid')) {
                    release.compilation[i].socialid = $(this).data('socialid');
                };

                release.compilation[i].releasetypeid = release.typeid;
                release.compilation[i].name = secureString($(this).find('.name').html());
                release.compilation[i].author = secureString($(this).find('.author').html());
                release.compilation[i].remix = secureString($(this).find('.remix').html());
                release.compilation[i].musictype = secureString($(this).find('.style').html());
                release.compilation[i].download = $(this).data('download');
                release.compilation[i].promocut = $(this).data('promocut');
                release.compilation[i].inplaylist = $(this).data('inplaylist') ? $(this).data('inplaylist') : null;
                release.compilation[i].active = $(this).find('.hide-item').attr('checked') ? 0 : 1;
                release.compilation[i].coverid = $(this).find('.preview').attr('rel') ? $(this).find('.preview').attr('rel') : null;
                release.compilation[i].cover = $(this).find('.preview').attr('src') == defaultCoverUrl ? '' : $(this).find('.preview').attr('src');
            });

             release.compilation.reverse();

            $.ajax({
                type: 'POST',
                url: '/admin/updatecompilation/',
        ...
        function init() {
            ...
            if (release.compilation) {
                ...
                $.each(release.compilation, function(i, item) {
