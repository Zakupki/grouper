...
public function getRequestcnt(){
            ...
            $result=$db->queryFetchAllFirst('
                        SELECT COUNT(*) FROM `z_public_group`
                        INNER JOIN z_group
                        ON z_group.id=z_public_group.`groupid` AND z_group.userid='.tools::int($_SESSION['User']['id']).'
                        WHERE z_public_group.new=1
                        ');
            if($result[0])
            return $result[0];
    }
...
public function getRequests(){
	...
	$result=$db->queryFetchAllAssoc('
					SELECT 
					  ...
					  IF(NOW()>z_public.date_start,1,0) AS passed,
					  "РџР°Р±Р»РёРє" AS requesttypename,
					  z_public_site.publicid AS requestid,
					  z_public_site.confirm,
					  z_public_site.status,
					  z_public.date_create,
					  z_public.date_start,
					  z_public.brandid,
					  z_public_site.new,
					...
					FROM
					  ...
					  INNER JOIN
					  z_public 
					  ON z_public.id = z_public_site.publicid 
					  INNER JOIN
					  z_brand 
					  ON z_public.brandid = z_brand.id
	...
	$result=$db->queryFetchAllAssoc('
                SELECT 
                  IF(z_public_group.publicid, 3, 0) AS requesttype,
                  IF(NOW() > z_public.date_start, 1, 0) AS passed,
                  "Публикация" AS requesttypename,
                  z_public_group.confirm,
                  z_public.date_create,
                  z_public.date_start,
                  z_public_group.status,
                  z_public_group.id AS requestid,
                  z_public_group.new,
                  z_public_report.`id` AS hasreport,
                  z_group.name as groupname
                FROM
                  z_public 
...
public function acceptRequest($id,$requesttype,$action){
	...
	switch ($requesttype) {
        ...
        case 3:
            $db->exec('
            UPDATE z_public_group
            INNER JOIN z_group
            ON z_group.id=z_public_group.groupid AND z_group.userid='.tools::int($_SESSION['User']['id']).'
            SET 
			status='.$confirmacts[$action].'
			WHERE z_public_group.id='.tools::int($id).'
            ');
            if(tools::int($action)==1)
            echo json_encode(array('url'=>'/user/requests/','status'=>'Вы добавили новый паблик. Разместите материалы на страницах ваших соцсетей и отправьте отчеты заказчику. Возможность добавить отчет появится по прошествию даты публикации.'));
            break;
        case 4:
        ...
        foreach($data['id'] as $k=>$reportid){
            ...
            if($reportid>0){
              if(strlen(trim($data['link'][$k]))>0)
                  $db->exec('UPDATE z_public_report SET link="'.$data['link'][$k].'" WHERE id='.tools::int($reportid).' AND userid='.tools::int($_SESSION['User']['id']).' AND publicrequestid='.tools::int($data['requestid'][$k]).'');
              else
                  $db->exec('DELETE FROM z_public_report WHERE id='.tools::int($reportid).'');
            }else{
                  $db->exec('INSERT INTO z_public_report (userid, link, publicrequestid) VALUES
                  ('.tools::int($_SESSION['User']['id']).', "'.tools::str($data['link'][$k]).'", '.tools::int($data['requestid'][$k]).')
                  ');
			}
        }
			if($reportid>0)
            ...
            ON z_group.id=z_public_group.groupid AND z_group.userid='.tools::int($_SESSION['User']['id']).'
            SET 
			z_public_group.date_update=NOW()
            WHERE z_public_group.id='.tools::int($requestid).'
            ');
...
public function markRead($requesttype,$requestid){
    ...
    switch ($requesttype) {
        ...
        case 2:
            ...
            break;
        case 3:
            $db->exec('UPDATE z_public_group SET new=2 WHERE id='.tools::int($requestid).'');
            break;
        case 4:
