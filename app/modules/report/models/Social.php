...
public function disconnect($id){
    $db=db::init();
    echo 'DELETE FROM z_social_account WHERE id='.tools::int($id).' AND userid='.tools::int($_SESSION['User']['id']);
    if($db->exec('DELETE FROM z_social_account WHERE id='.tools::int($id).' AND userid='.tools::int($_SESSION['User']['id'])))
        return true;
}
