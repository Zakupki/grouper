...
Class Reccount Extends Basemodel {
	...
	$result=$db->queryFetchAllAssoc('
				SELECT 
				  ...
				  z_site_social.url as vkontakte,
				  z_site_social2.url as facebook,
				  z_social_stats.value AS facebook_count,
				  z_social_stats2.value AS vkontakte_count
				FROM
				  z_site 
				  INNER JOIN
				  z_user 
				  ...
				  ON z_social_stats.siteid=z_site.id AND z_social_stats.statstypeid=4 AND z_social_stats.date = (select max(date) from z_social_stats where z_social_stats.statstypeid=4)
				  LEFT JOIN z_site_social z_site_social2
                  ON z_site_social2.siteid=z_site.id AND z_site_social2.socialid in(255)
                  LEFT JOIN z_social_stats z_social_stats2
                  ON z_social_stats2.siteid=z_site.id AND z_social_stats2.statstypeid=6 AND z_social_stats2.date = (select max(date) from z_social_stats where z_social_stats.statstypeid=6)
