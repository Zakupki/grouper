											...
											<div class="sct_right">
												...
												<div class="table_wrapper">
													...
													<table cellpadding="0" cellspacing="0" width="100%">
														...
														<? if(is_array($this->view->sociallist)){
														$linecss=array(0=>'first', 1=>'second');
														$cnt=0;
														foreach($this->view->sociallist as $social){
														if($cnt==2)
														$cnt=0;
														if($social['domain'])
														$social['domain']='<a target="_blank" href="http://'.$social['domain'].'/">http://'.$social['domain'].'</a>';
														else
														$social['domain']='<a target="_blank" href="http://r'.$social['id'].'.reactor.ua/">http://r'.$social['id'].'.reactor.ua</a>';
														if($social['url'])
														$social['url']='<a class="product_thumb" href="/admin/social/'.$social['id'].'/"><img width="20" height="20" src="'.$social['url'].'"/></a>';
														?>
														<tr class="<?=$linecss[$cnt];?>">
															<td><?=$social['id'];?></td>
															<td class="photo"><?=$social['url'];?></td>
															<td><a class="product_name" href="/admin/social/<?=$social['id'];?>/"><?=$social['name'];?></a></td>
															<td><?=($social['new']==1)?'Новое':'';?></td>
														</tr>
														<?
														$cnt++;
														}
