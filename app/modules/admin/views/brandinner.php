<div class="section">
						<div class="title_wrapper">
							<h2>Бренд: <?=$this->view->brandinner['name'];?></h2>
											...
											<div class="sct_right">
												<form action="/admin/brands/updatebrandinner/" method="POST" enctype="multipart/form-data" class="search_form general_form">
													<fieldset>
														...
														<?if(!$this->view->brandinner['id'])
															$this->view->brandinner['type']=1;
														?>
														<input type="hidden" name="type" value="<?=$this->view->brandinner['type'];?>" />
														...
														<? if($this->view->brandinner['type']==0){?>
														<div class="row">
															<label>Изменить и объеденить с:</label>
															<div class="inputs">
																<span class="input_wrapper blank">
																	<select name="mergeto">
																		<option></option>
																		<? foreach($this->view->brandlist as $brandl){
																		if($brandl['id']!=$this->view->brandinner['id'])?>
																		<option value="<?=$brandl['id'];?>"><?=$brandl['name'];?></option>
																		<?}?>
																	</select>
																</span>
															</div>
														</div>
														<?} if($this->view->brandinner['type']==1){?>
														<div class="row">
												...
												<ul class="system_messages">
													<li class="yellow"><span class="ico"></span><strong class="system_title">Данный бренд не является клиентом и используется только в анкете клуба!</strong></li>
												</ul>
												<?}?>
											</div>
